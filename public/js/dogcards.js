document.addEventListener('DOMContentLoaded', () => {
    ...
    card.innerHTML = `
      <header class="card-header">
        ...
        <button class="heart-button ${pet.saved ? 'saved' : ''}">
          <i class="${
            pet.saved ? 'fas' : 'far'
          } fa-heart"></i> <!-- Font Awesome Heart Icon -->
        </button>
      </header>
      <div class="card-image">
        <figure class="image is-4by3">
          <img src="${dogImage}" alt="Pet image for ${pet.name}">
        </figure>
      </div>
      <div class="card-content">
        <div class="content">
          ...
          <button class="button more-info" id="more-info-${
            pet.name
          }">More Info</button>
          <div class="additional-info is-hidden" id="additional-info-${
            pet.name
          }">
  ...
  `;
    ...
    moreInfoBtn.addEventListener('click', (event) => {
      const additionalInfoDiv = card.querySelector(
        `#additional-info-${pet.name}`
      );
      additionalInfoDiv.classList.toggle('is-hidden');
      event.target.textContent = additionalInfoDiv.classList.contains(
        'is-hidden'
      )
        ? 'More Info'
        : 'Less Info';
  ...
  function updateSavedPetsSection(petCard, pet) {
    ...
    savedPetsSection.style.display =
      savedPetsContainer.children.length > 0 ? 'block' : 'none';
  }
